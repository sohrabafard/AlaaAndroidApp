...
public class PlayListAdapter extends RecyclerView.Adapter<PlayListAdapter.VideoHolder> {
    ...
    private ArrayList<PlayList> itemsList;
    private Context mContext;
    private OnItemClickListener onItemClickListener;

    public PlayListAdapter(Context context, ArrayList<PlayList> itemsList) {
    ...
    public void onBindViewHolder(final VideoHolder holder, final int position) {
        ...
        final PlayList item = itemsList.get(position);

        holder.txtTitle.setText(item.getTitle());

        int width = AppConfig.width / 3;
        width -= 8;
